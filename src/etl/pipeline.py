...
def clean_source_data(df, file_name):
    ...
    if file_name in {
        ...
        mask = df["company_id"].astype(str).str.strip().str.upper().eq("ABB")
        ...
        if not abb.empty:
            ...
            if removed:
                print(f"{file_name}: " f"removed {removed} conflicting ABB rows")
    ...
    if file_name in {
        ...
        if removed:
            print(f"{file_name}: " f"removed {removed} identical duplicate rows")
    ...
    # ------------------------------------------------------------------
    if file_name == "profitandloss.xlsx" and {"company_id", "year", "sales"}.issubset(
        df.columns
    ):
        mask = (
            df["company_id"].astype(str).str.strip().str.upper().eq("ADANIENSOL")
            & df["year"].astype(str).str.strip().str.upper().eq("MAR 2014")
            & pd.to_numeric(
                df["sales"],
                errors="coerce",
            ).eq(0)
    ...
    if "company_id" in df.columns:
        mask = df["company_id"].astype(str).str.strip().str.upper().eq("AGTL")
        ...
        if corrected:
            ...
            print(f"{file_name}: " f"corrected {corrected} AGTL rows to ATGL")
...
def run_pipeline() -> None:
    ...
    print("Running N100 ETL pipeline...")
